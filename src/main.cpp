...
const string strMessageMagic = "Litas Signed Message:\n";
...
const int64 nInterval = 10; // 10 minutes for VLT
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    if (pindexLast->nHeight > nLifeWindow && count < nLifeWindow/4) {
        ...
        printf("Retarget: **** vLitas Temporal Retargeting **** %d/%d : factor = %d\n",
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // vLitas: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "12/21/2014 Litas is still in Lithuania";
        CTransaction txNew;
	txNew.vin.resize(1);
	txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // vLitas: increase each by adding 1 to vertcoin's value.
//250,191,181,218
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("vLitasMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("vLitasMiner : ProcessBlock, block not accepted");
...
void static vLitasMiner(CWallet *pwallet)
{
    printf("vLitasMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vlitas-miner");
    ...
    try { loop {
        ...
        printf("Running vLitasMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nGenerateThreads; i++)
        minerThreads->create_thread(boost::bind(&vLitasMiner, pwallet));
}
